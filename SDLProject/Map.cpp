...
void Map::build()
{
    for (int y_coord = 0; y_coord < m_height; y_coord++)
    {
        for (int x_coord = 0; x_coord < m_width; x_coord++)
        {
            ...
            float u_coord = (float)(tile_index % m_tile_count_x) / m_tile_count_x;
            float v_coord = (float)(tile_index / m_tile_count_x) / m_tile_count_y;
            v_coord = 1.0f - v_coord - tile_height;

            float x_offset = -(m_tile_size / 2.0f);
            float y_offset = (m_tile_size / 2.0f);
